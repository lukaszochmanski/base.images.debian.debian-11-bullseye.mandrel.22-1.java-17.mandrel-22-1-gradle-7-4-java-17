...
if [ ! -z "$CODEBUILD_BUILD_IMAGE" ]
then
  echo CODEBUILD_BUILD_IMAGE
  echo The CodeBuild build image identifier.
  echo $CODEBUILD_BUILD_IMAGE
  echo
fi
...
if [ ! -z "$CODEBUILD_WEBHOOK_BASE_REF" ]
then
...
if [ ! -z "$CODEBUILD_WEBHOOK_MERGE_COMMIT" ]
then
...
if [ ! -z "$CODEBUILD_WEBHOOK_PREV_COMMIT" ]
then
...
IS_INCORRECTLY_CHECKED_OUT_BRANCH=`git branch --contains HEAD | grep "* (no branch)"`
